...
<html>
	...
	<body>
		...
		<?php
					require 'conection.php';
					$sql1 = "SELECT id_noticia,titulo,sinopsis,portada,fecha FROM noticia WHERE destacada=1 AND tamaño=0 ORDER BY prioridad DESC LIMIT 10";
					$sql16 = "SELECT id_noticia,titulo,sinopsis,portada,fecha FROM noticia WHERE destacada=1 AND tamaño=1 ORDER BY prioridad DESC LIMIT 10";
					$texto26=mysqli_query($con,$sql16) or die(mysqli_error($con));
					
					$texto2=mysqli_query($con,$sql1) or die(mysqli_error($con));
					if(count($texto2)!=0 || count($texto26)!=0){
						echo "<section class='notisec'>";
						$p=rand(1, 3);
						while($noticiadestacada = mysqli_fetch_array($texto26)){
						...
						while($noticiadestacada = mysqli_fetch_array($texto2)){
							...
							if($x==1){
								...
								echo "<div class='contenedor-img3 ejemplo-3'><img src='".$noticiadestacada['3']."' /><div class='mascara3'><p class='date'>".$daa."</p><h2 class='nose4'>".$titulo."</h2><p>".$sinopsis."</p><a href='display.php?noticia=".$noticiadestacada['0']."' class='link'>Leer Mas</a><div class='icon'><a href='modificarnoti.php?noticia=".$noticiadestacada[0]."'><i id='edi' class='fa fa-pencil'></i></a><a href='quickdeletenoti.php?noticia=".$noticiadestacada['0']."'><i id='bas' class='fa fa-trash'></i></a></div></div></div>";
						...
						if(is_numeric($publicacion[0])){
							...
							if(!empty($filasz[1])){
								if(detectarURL($filasz[1])){
									echo "<a href='".$filasz[1]."' target='_blank'><img src=".$filasz[0]." class='vinculorelacionado'/></div><a/>";
								}else{
