		...
		<section class="notise">
				<?php
					...
					if(isset($_GET['orden'])){
						...
						if($order==1){
							if($bol==1){
								$sql1 = "SELECT id_noticia,titulo,sinopsis,portada,prioridad,fecha FROM noticia WHERE texto LIKE '%".$palabra."%' OR titulo LIKE '%".$palabra."%' ORDER BY fecha DESC limit $pagina1,8";
							}else{
								$sql1 = "SELECT id_noticia,titulo,sinopsis,portada,prioridad,fecha FROM noticia WHERE texto LIKE '%".$palabra."%' OR titulo LIKE '%".$palabra."%' ORDER BY fecha ASC limit $pagina1,8";
							}
						}else if($order==2){
							if($bol==1){
								$sql1 = "SELECT id_noticia,titulo,sinopsis,portada,prioridad,fecha FROM noticia WHERE texto LIKE '%".$palabra."%' OR titulo LIKE '%".$palabra."%' ORDER BY titulo ASC limit $pagina1,8";
							}else{
								$sql1 = "SELECT id_noticia,titulo,sinopsis,portada,prioridad,fecha FROM noticia WHERE texto LIKE '%".$palabra."%' OR titulo LIKE '%".$palabra."%' ORDER BY titulo DESC limit $pagina1,8";
							}
						}else{
							if($bol==1){
								$sql1 = "SELECT id_noticia,titulo,sinopsis,portada,prioridad,fecha FROM noticia WHERE texto LIKE '%".$palabra."%' OR titulo LIKE '%".$palabra."%' ORDER BY prioridad DESC limit $pagina1,8";
							}else{
								$sql1 = "SELECT id_noticia,titulo,sinopsis,portada,prioridad,fecha FROM noticia WHERE texto LIKE '%".$palabra."%' OR titulo LIKE '%".$palabra."%' ORDER BY prioridad ASC limit $pagina1,8";
							}
						}
					}else{
						$sql1 = "SELECT id_noticia,titulo,sinopsis,portada,prioridad,fecha FROM noticia WHERE texto LIKE '%".$palabra."%' OR titulo LIKE '%".$palabra."%'  ORDER BY prioridad DESC limit $pagina1,8";
					...
					while($noticiadestacada = mysqli_fetch_array($texto2)){
						...
						$titulo=cortar($noticiadestacada['1'],27);
							...
							if($x==1){
								echo "<div class='contenedor-img7 ejemplo-3' id='chico'><img src='admin/".$noticiadestacada['3']."' /><div class='mascara7'><p class='date'>".$daa."</p><h2>".$titulo."</h2><p class='date'>".$sinopsis."</p><a href='display.php?noticia=".$noticiadestacada['0']."' class='link'>Leer Mas</a></div></div>";
								
							}elseif($x==2){
								echo "<div class='contenedor-img8 ejemplo-3' id='chico'><img src='admin/".$noticiadestacada['3']."' /><div class='mascara8'><p class='date'>".$daa."</p><h2 class='nose3'>".$titulo."</h2><p>".$sinopsis."</p><a href='display.php?noticia=".$noticiadestacada['0']."' class='link'>Leer Mas</a></div></div>";
								
							}elseif($x==3){
								echo "<div class='contenedor-img9 ejemplo-3' id='chico'><img src='admin/".$noticiadestacada['3']."' /><div class='mascara9'><p class='date'>".$daa."</p><h2>".$titulo."</h2><p>".$sinopsis."</p><a href='display.php?noticia=".$noticiadestacada['0']."' class='link'>Leer Mas</a></div></div>";
