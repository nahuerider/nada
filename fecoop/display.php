 ...
 <html>
	 ...
	 <div >
	 <div class="menu_bar">
	 <img src="logonav.svg" class="logo3" />	
			...
			<div class="fr-view">
				<?php 
					...
					if(empty($fila)){
						...
						echo "<section class='cuadronoticia'><img src='".$fila[3]."' class='detalleportada' /><center><h2 class='titulodetalle'>".$titulo."</h2></center><p class='date'>".$daa."</p><div class='noticiadetalle'>".$fila[2]."</div><div class='textodetalle'>".$fila[1]."</div><div class='compartir'><iframe id='jinju' src='' width='123' height='20' style='border:none;overflow:hidden' scrolling='no' frameborder='0' allowTransparency='true'></iframe><a href='https://twitter.com/share?ref_src=twsrc%5Etfw' class='twitter-share-button' data-show-count='false'>Tweet</a><script async src='https://platform.twitter.com/widgets.js' charset='utf-8'></script></div></section>";
					}
				?>
								...
								if(!empty($check)){
									...
									while($check=mysqli_fetch_array($consulta6)){
										...
										while($entes = mysqli_fetch_array($consulta5)){
											echo "<div class='cajaasociados'><img class='imgasociados' src='".$entes[2]."'/><div class='overlay'><div class='text'>".substr($entes[1],0,30)." <br><a href='verasociado.php?asociado=".$entes[0]."' class='link'>VER</a></div></div></div>";
										}
									}
									echo "</section>";		
								}else{
